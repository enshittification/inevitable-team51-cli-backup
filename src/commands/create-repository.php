...
class Create_Repository extends Command {
	...
	protected function execute( InputInterface $input, OutputInterface $output ): int {
		...
		if (  'project' === $this->repo_type || 'plugin' === $this->repo_type ) {
			...
			// The `phpcs` and `phpcbf` commands return non-zero if they found issues to fix, which triggers an exception...so we must ensure that we don't exit on non-zero.
			run_system_command( array( 'composer', 'run-script', 'format:php' ), TEAM51_CLI_ROOT_DIR . "/scaffold/$this->repo_slug", false ); // Fix any automated formatting issues generated by the operations above.
		}
